            ...
            <div class="panel panel-bd lobidrag" style='height:80px; border: 3px solid #d7d4d6;'>
               ...
               <table class="table" align="center" style="overflow-x: unset;position: relative; left: 150px;">
                  <tr style='text-align:center;font-weight:bold;' class="filters">
                     ...
                     <input type='hidden' name='url' value='<?php echo $tax_n; ?>'/>
...
<?php $c = 1;  foreach ($merged_reports as $time_sheet_id => $data) : ?>
    <?php foreach ($data['state_tax'] as $state_tax) : ?>
        <tr>
            <td><?php echo $c; ?></td>
            <td><?php echo $state_tax['first_name'] . ' ' . $state_tax['last_name']; ?></td>
            ...
            <td><?php echo isset($state_tax['amount']) ? $state_tax['amount'] : '0'; ?></td>
            <td>
                <?php 
                ...
                foreach ($data['living_state_tax'] as $living_state_tax) {
                    if ($living_state_tax['time_sheet_id'] == $time_sheet_id) {
                        echo isset($living_state_tax['amount']) ? $living_state_tax['amount'] : '0';
...
$('#fetch_tax').submit(function (event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.forEach(function (mergedTaxData) {
                var state_tax = mergedTaxData.working_employee || {};
                var living_state_tax = mergedTaxData.living_employee || {};
                var employer_state_tax = mergedTaxData.working_employer || {};
                var employer_living_state_tax = mergedTaxData.living_employer || {};

                var row = "<tr>";
                row += "<td>" + c + "</td>";
                row += "<td>" + state_tax['first_name'] + ' ' + state_tax['last_name'] + "</td>";
                 row += "<td>" + state_tax['employee_tax'] + "</td>";
                  row += "<td>" + state_tax['state_tx'] + "</td>";
                   row += "<td>" + state_tax['living_state_tax'] + "</td>";
                row += "<td>" + state_tax['time_sheet_id'] + "</td>";
                row += "<td>" + state_tax['month'] + "</td>";
                row += "<td>" + (state_tax['amount'] ? state_tax['amount'] : '0') + "</td>";
                row += "<td>" + (living_state_tax['amount'] ? living_state_tax['amount'] : '0') + "</td>";
                row += "<td>" + (employer_state_tax['amount'] ? employer_state_tax['amount'] : '0') + "</td>"; // Fetch employer's working state tax
                row += "<td>" + (employer_living_state_tax['living_state_tax'] ? employer_living_state_tax['living_state_tax'] : '0') + "</td>"; // Fetch employer's living state tax
                row += "</tr>";
